...
class CollectorLivewire extends Component
{
    ...
    public function builder()
    {
        ...
        $collector = Collector::join('users', 'collectors.user_id', '=', 'users.id')
            ...
            ->select('collectors.user_id', 'rice_seasons.name as riceSeasonName', 'collectors.phone_no', 'collectors.id', 'collectors.ai_range', 'collectors.village', 'collectors.gps_lati', 'collectors.gps_long', 'collectors.rice_variety', 'collectors.date_establish', 'users.name', 'users.email', 'districts.name as dname', 'as_centers.name as asname', 'ai_ranges.name as ainame')
