...
class PestRainComparison extends Component
{
    ...
    public function mount()
    {
        ...
        $this->selectedSeasons = $this->seasons->pluck('id')->toArray(); // default all
        $this->fetchChartData();
    }
    ...
    {
        if (in_array($propertyName, ['selectedPest', 'districtId', 'selectedSeasons', 'showRain'])) {
            $this->fetchChartData();
        }
    }
    ...
    protected function getPestData(): array
    {
        ...
        foreach ($this->seasons as $season) {
            if (!in_array($season->id, $this->selectedSeasons)) {
                continue;
            }
            ...
            foreach ($weeks as $weekStart) {
                ...
                $rainDays[] = $weeklyPests['rainnyDays'] ?? 0;
    ...
    {
        return view('livewire.graph.pest-rain-comparison');
    }
}
