<x-app-layout>
    <div class="space-y-6 m-2 md:m-6">
        <!-- Header -->
        <x-headings.top-heading title="Pest Data Overview" subtitle="Detailed collected records" icon="fas fa-file-alt" buttonText="Back"
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
            ...
            @foreach ($items as $item)
                <div class="bg-white dark:bg-slate-800 rounded-xl p-5 border border-slate-200 dark:border-slate-700 shadow-sm flex items-center gap-4 hover:shadow-md transition-shadow">
                    <div class="w-12 h-12 rounded-lg {{ $item['bgClass'] }} {{ $item['textClass'] }} flex items-center justify-center flex-shrink-0 text-xl">
                        <i class="fas {{ $item['icon'] }}"></i>
                    </div>
                    <div>
                        <p class="text-sm font-medium text-slate-500 dark:text-slate-400">{{ $item['label'] }}</p>
                        <p class="text-lg font-bold text-slate-900 dark:text-white">{{ $item['value'] }}</p>
                    </div>
                </div>
            @endforeach
        </div>
        ...
        <!-- Pest Table Card -->
        <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm rounded-xl overflow-hidden">
                    ...
                    <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                        @foreach ($pestsData as $pestData)
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                                <td class="px-6 py-4 font-medium text-slate-900 dark:text-white">{{ str_replace('_', ' ', $pestData->pest_name) }}</td>
                                @for ($i = 1; $i <= 10; $i++)
                                    <td class="hidden px-2 py-4 text-center text-slate-600 dark:text-slate-300 sm:table-cell">
                                ...
                                <td class="px-6 py-4 text-center">
                                    @php
                                        $colorClass = match ($pestData->code) {
                                            0 => 'bg-emerald-100 text-emerald-800 dark:bg-emerald-500/20 dark:text-emerald-400',
                                            1 => 'bg-green-100 text-green-800 dark:bg-green-500/20 dark:text-green-400',
                                            3 => 'bg-yellow-100 text-yellow-800 dark:bg-yellow-500/20 dark:text-yellow-400',
                                            5 => 'bg-orange-100 text-orange-800 dark:bg-orange-500/20 dark:text-orange-400',
                                            ...
                                            default => 'bg-slate-100 text-slate-800 dark:bg-slate-500/20 dark:text-slate-400',
                                        };
                                    @endphp
                                    <span class="inline-flex items-center justify-center px-3 py-1 text-xs font-bold rounded-full {{ $colorClass }}">
                                        {{ $pestData->code }}
                                    </span>
                                </td>
                            </tr>
        ...
        @if ($commonData->otherinfo)
            <div class="bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-100 dark:border-indigo-500/20 rounded-xl p-5 shadow-sm">
                ...
                <p class="text-indigo-800 dark:text-indigo-300 leading-relaxed whitespace-pre-wrap">{{ $commonData->otherinfo }}</p>
            </div>
        @endif
    </div>
</x-app-layout>
