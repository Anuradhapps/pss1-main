<x-app-layout>
    ...
    <x-form method="POST" action="{{ route('pestdata.update',$commonData->id) }}">
        ...
        <div class="grid grid-cols-2 gap-4 mb-6">
            ...
            <div class="col-span-2 sm:col-span-1">
                <x-form.input type="number" name="temperature" label="Temperature:">{{ $commonData->temperature }}
                </x-form.input>
            </div>
            <div class="col-span-2 sm:col-span-1">
                <x-form.select id="growth_s_c" label="Growth Stage Code" class="block mt-1 w-full" name="growth_s_c">
                    <option value="">-- Select code --</option>
                    @for ($i = 1; $i <= 10; $i++) <option value="{{ $i }}" {{ $i==$commonData->growth_s_c ? 'selected' :
                        ''}}>{{ $i }}</option>
                        @endfor
                </x-form.select>
            </div>
            <div class="col-span-2 sm:col-span-1">
                <x-form.select id="growth_s_c" label="Number of Rainy Days:" class="block mt-1 w-full"
                    ...
                    @for ($i = 1; $i <= 7; $i++) <option value="{{ $i }}" {{ $i==$commonData->numbrer_r_day ? 'selected'
                        : ''}}>{{ $i }}</option>
                        @endfor
                </x-form.select>
            </div>
            ...
            <div class="toggleDiv hidden bg-gray-450 p-4 border border-black rounded-md">
                ...
                <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-5 xl:grid-cols-10 gap-4">
                    ...
                    <div class="col-span-1">
                        <x-form.input type="number" name="{{ $pest->pest_name }}all_location" label="All Location:">{{
                            $pest->total }}
                        </x-form.input>
                    </div>
