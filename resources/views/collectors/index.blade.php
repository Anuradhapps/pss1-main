<x-app-layout>
    ...
    <div>
        ...
        <!-- Filter Toolbar -->
        <div x-data="{ showFilters: false }" class="mb-6 px-2">
            <div class="flex items-center justify-between">
                <button @click="showFilters = !showFilters"
                    ...
                    <div class="w-6 h-6 rounded bg-slate-100 dark:bg-slate-900 flex items-center justify-center text-slate-500 dark:text-slate-400 group-hover:text-primary transition-colors">
                ...
                <form method="GET" action="{{ route('collector.index') }}" class="p-5 flex flex-col gap-5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                        ...
                        <div class="flex flex-col">
                            <label class="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Rice Season</label>
                            <div class="relative">
                                <i class="fas fa-leaf absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 text-sm"></i>
                                <select name="season"
                        ...
                        <!-- District -->
                        <div class="flex flex-col">
                            <label class="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">District</label>
                            <div class="relative">
                        ...
                        <!-- Established Date -->
                        <div class="flex flex-col">
                            <label class="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Established Date</label>
                            <div class="relative">
                        ...
                        <!-- Created At -->
                        <div class="flex flex-col">
                            <label class="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Created At</label>
                            <div class="relative">
                                <i class="far fa-calendar-plus absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 text-sm z-10"></i>
        ...
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-3 p-2">
            @foreach ($collectors as $collector)
                <div class="flex flex-col p-5 transition-all duration-300 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm hover:shadow-lg group relative overflow-hidden">
                    
                    <!-- Top Ribbon & Actions -->
                    <div class="flex items-center justify-between mb-5 gap-3">
                        <!-- Number Badge -->
                        <span class="flex items-center justify-center w-12 h-10 text-base font-black text-white bg-slate-800 dark:bg-slate-700 rounded-xl shadow-sm flex-shrink-0 border border-slate-700 dark:border-slate-600">
                            #{{ $collectors->count() - $loop->index }}
                        </span>
                        
                        <!-- High Contrast Season Badge -->
                        <div class="flex-1 flex items-center justify-center px-4 py-2 text-sm font-black tracking-wider uppercase text-white bg-gradient-to-r from-emerald-600 to-teal-500 rounded-xl shadow-md border border-emerald-500/50">
                    ...
                    <div class="mb-4 flex flex-wrap items-center gap-2 text-sm">
                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 font-semibold">
                            <i class="fas fa-map-marker-alt text-xs opacity-70"></i> {{ $collector->getDistrict->name }}
                        </span>
                    ...
                    <!-- Metadata Grid -->
                    <div class="flex-1 bg-slate-50 dark:bg-slate-900/50 rounded-xl p-4 border border-slate-100 dark:border-slate-700/50 mb-5">
                        <div class="grid grid-cols-2 gap-y-3 gap-x-4 text-sm">
                            <div class="flex flex-col">
                                <span class="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Created At</span>
                                <span class="font-medium text-slate-700 dark:text-slate-200">{{ \Carbon\Carbon::parse($collector->created_at)->format('Y-m-d H:i') }}</span>
                            </div>
                            <div class="flex flex-col">
                                <span class="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Updated At</span>
                                <span class="font-medium text-slate-700 dark:text-slate-200">{{ \Carbon\Carbon::parse($collector->updated_at)->format('Y-m-d H:i') }}</span>
                            </div>
                            <div class="flex flex-col">
                                <span class="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Rice Variety</span>
                            ...
                            <div class="flex flex-col">
                                <span class="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Established Date</span>
                                <span class="font-medium text-slate-700 dark:text-slate-200">{{ $collector->date_establish }}</span>
                            </div>
                            <div class="flex flex-col">
                                <span class="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Method</span>
                                <span class="font-medium text-slate-700 dark:text-slate-200">{{ $collector->established_method ?? 'N/A' }}</span>
                            </div>
                            <div class="flex flex-col">
                                <span class="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Pest Data Count</span>
                                <span class="font-bold text-primary flex items-center gap-1.5">
                                    <i class="fas fa-database text-primary opacity-70"></i> {{ $collector->commonDataCollect->count() }}
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- Action Buttons -->
                    <div class="grid grid-cols-2 gap-3 mt-auto pt-4 border-t border-slate-100 dark:border-slate-700/50">
