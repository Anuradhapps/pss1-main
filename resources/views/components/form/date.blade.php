...
<div class="mb-5">
    ...
    <div class="rounded-md shadow-sm">
        <input placeholder="dd-mm-yyyy" x-data x-init="flatpickr($refs.input, {{ json_encode((object) $options) }});" x-ref="input" type="text" id="{{ $name }}"
            ...
            {{ $attributes->merge(['class' => ' block w-full bg-gray-600 text-gray-200 placeholder-gray-200 border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-light-blue-500 focus:border-light-blue-500 sm:text-sm']) }}>
        @error($name)
