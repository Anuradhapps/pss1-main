...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
    @foreach ($cards as $card)
        ...
        <a href="{{ $card['route'] }}"
            ...
            <div
                class="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 {{ $c['hoverBg'] }}">
            </div>

            {{-- Icon --}}
            <div
                class="relative flex items-center justify-center w-12 h-12 mb-3 rounded-xl {{ $c['bg'] }} 
            ...
            {{-- Title --}}
            <h2
                class="relative text-sm font-semibold text-slate-700 dark:text-slate-300 text-center leading-tight group-hover:text-slate-900 dark:group-hover:text-white transition-colors duration-200">
                {{ $card['title'] }}
            </h2>

            {{-- Active indicator dot --}}
            <div
                class="absolute top-3 right-3 w-2 h-2 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300 {{ $c['progress'] }}">
            </div>
