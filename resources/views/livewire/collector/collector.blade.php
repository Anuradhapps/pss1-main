...
<div x-data="{ deleteModalOpen: false, collectorToDelete: null, collectorName: '' }" class="space-y-6">
    <!-- Header -->
            ...
            <div class="p-5 flex flex-col gap-5">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                    ...
                    <div class="flex flex-col">
                        <label class="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Global Search</label>
                        <div class="relative">
                            <i class="fas fa-search absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 text-sm"></i>
                            <input type="search" wire:model.live.debounce.300ms="query" placeholder="Search by name, district or AI range..."
                    ...
                    <!-- Name Search Filter -->
                    <div class="flex flex-col">
                        <label class="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Specific Name Search</label>
                        <div class="relative">
                            <i class="fas fa-user absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 text-sm"></i>
                            <input type="text" wire:model.live.debounce.300ms="nameSearch" placeholder="Filter precisely by name..."
                                class="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-lg text-sm text-slate-900 dark:text-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors">
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col">
                        <label class="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Filter by Districts</label>
                        <select wire:model.live="selectedDistricts" multiple class="w-full px-3 py-2 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-lg text-sm text-slate-900 dark:text-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors custom-scrollbar" style="min-height: 100px;">
                            @if(isset($allDistricts))
                                @foreach($allDistricts as $district)
                                    <option value="{{ $district->id }}" class="py-1 px-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded">{{ $district->name }}</option>
                                @endforeach
                            @endif
                        </select>
                        <span class="text-[10px] text-slate-400 mt-1"><i class="fas fa-info-circle mr-1"></i>Hold Ctrl/Cmd to select multiple</span>
                    </div>
                </div>
                ...
                <div class="flex flex-col sm:flex-row gap-3 justify-end items-center pt-5 mt-2 border-t border-slate-100 dark:border-slate-700/50">
                    <button type="button" wire:click="$set('query', ''); $set('nameSearch', ''); $set('selectedDistricts', []);" @click="showFilters = false"
                ...
                <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-4">
                            <div class="w-12 h-12 flex shrink-0 items-center justify-center rounded-2xl font-black bg-gradient-to-br from-emerald-100 to-emerald-200 text-emerald-800 dark:from-emerald-900/50 dark:to-emerald-800/50 dark:text-emerald-400 ring-1 ring-emerald-300 dark:ring-emerald-700 shadow-sm">
                                {{ strtoupper(substr($collector->name, 0, 2)) }}
                            </div>

                            <div class="flex flex-col">
                                <h3 class="text-sm font-bold text-slate-900 dark:text-white group-hover:text-primary transition-colors">{{ $collector->name }}</h3>
                                ...
                                <div class="md:hidden flex flex-col gap-1 mt-2 text-[11px] text-slate-500 dark:text-slate-400">
                                    <span class="flex items-center gap-1.5"><i class="fas fa-map-marker-alt text-slate-400"></i> {{ $collector->dname }} &bull; {{ $collector->asname }}</span>
                                    <span class="flex items-center gap-1.5"><i class="fas fa-draw-polygon text-slate-400"></i> {{ $collector->ainame }}</span>
                                </div>
                            </div>
                    ...
                    <td class="px-6 py-4 hidden md:table-cell">
                        <div class="text-sm font-semibold text-slate-900 dark:text-slate-200">{{ $collector->dname }}</div>
                        <div class="text-[13px] text-slate-500 dark:text-slate-400 mt-0.5">{{ $collector->asname }}</div>
                    </td>
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center justify-center gap-2">
                            ...
                            @else
                                <span class="flex items-center justify-center w-8 h-8 rounded-lg bg-slate-100 dark:bg-slate-800/50 text-slate-300 dark:text-slate-600 border border-slate-200 dark:border-slate-700 cursor-not-allowed" 
            ...
            <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                <div x-show="deleteModalOpen"
                    ...
                    <div class="bg-white dark:bg-slate-900 px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                                <h3 class="text-lg font-bold leading-6 text-slate-900 dark:text-white" id="modal-title">Delete Collector</h3>
                                <div class="mt-2">
                                    <p class="text-sm text-slate-500 dark:text-slate-400">
                                        Are you sure you want to permanently delete collector <strong x-text="collectorName" class="text-slate-900 dark:text-white"></strong>? All of their data will be permanently removed. This action cannot be undone.
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="bg-slate-50 dark:bg-slate-800/50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 border-t border-slate-200 dark:border-slate-700">
                        <form x-bind:action="`/admin/collector/${collectorToDelete}/destroy`" method="POST" class="inline-block m-0 p-0 w-full sm:w-auto">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="inline-flex w-full justify-center rounded-xl bg-rose-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-rose-500 sm:ml-3 sm:w-auto transition-colors">Yes, Delete</button>
                        </form>
        ...
        <div class="p-5 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Map view of all collector assigned regions.</p>
                </div>
            </div>
        </div>
