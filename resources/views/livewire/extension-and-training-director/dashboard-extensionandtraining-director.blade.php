...
<div class="min-h-screen bg-white dark:bg-slate-900 text-slate-900 dark:text-white p-2 space-y-2">
    <!-- Top Heading -->
    <x-headings.top-heading title="Inter-Provincial Dashboard" icon="fas fa-clipboard"
        class="bg-gradient-to-r from-green-900 to-green-900  text-slate-900 dark:text-white p-0" />
    ...
    <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-2 mt-2">
        ...
        @if ($selectedSeason || $selectedDistrict || $searchNumber)
            <div
                ...
                <div
                    class="ml-3 bg-green-600 w-10 h-10 sm:w-11 sm:h-11 flex items-center justify-center text-slate-900 dark:text-white text-sm sm:text-base font-bold border border-green-700">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center gap-2">
        <div class="flex flex-wrap sm:justify-end gap-3 w-full sm:w-auto items-center">
            <input type="text" wire:model.debounce.500ms="search" placeholder="Search by name"
                class="rounded-none px-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 placeholder-gray-500 focus:ring-2 focus:ring-blue-600 transition" />
            ...
            <select wire:model="selectedDistrict"
                class="rounded-none px-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 focus:ring-2 focus:ring-blue-600 transition">
            ...
            <button wire:click="resetFilters"
                class="rounded-none px-4 py-2 bg-red-700 hover:bg-red-800 text-slate-900 dark:text-white shadow transition transform hover:scale-105">
                Reset
            </button>
            <div class="relative group inline-block">
                <button wire:click="downloadCollectorsList"
                    class="flex items-center gap-2 rounded-none px-4 py-2 bg-green-700 hover:bg-green-800 text-slate-900 dark:text-white shadow transition transform hover:scale-105">
                ...
                <!-- Tooltip -->
                <span
                    class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-max text-xs text-slate-900 dark:text-white bg-slate-50 dark:bg-slate-800 px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition duration-300 whitespace-nowrap">
                ...
                <table class="min-w-full bg-gray-950 border border-green-900 rounded-sm shadow-sm">
                    ...
                    <tbody class="text-sm text-green-100">
                        @foreach ($filteredCollectors as $collector)
                            <tr
                                class="border-b border-green-900 hover:bg-green-950 bg-slate-50 dark:bg-slate-800 hover:text-slate-900 dark:text-white transition duration-200 ease-in-out cursor-pointer">
                                <td class="px-4 py-2 font-medium whitespace-nowrap text-slate-900 dark:text-white">
                                ...
                                <td class="px-4 py-2 text-right">
                                    <button wire:click="viewCollector({{ $collector->id }})"
                                        class="inline-flex items-center px-3 py-1.5 bg-green-600 hover:bg-green-700 text-slate-900 dark:text-white text-xs font-semibold rounded transition transform hover:scale-105">
                                        View
                                    </button>
                                </td>
                            </tr>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-2 mt-6">
                ...
                <div>
                    ...
                    <div class="text-sm text-slate-600 dark:text-slate-300 mt-1">
                        @if ($filteredCollectorsBy->isNotEmpty())
                            <span class="font-semibold text-slate-900 dark:text-white text-sm sm:text-base">
                                {{ $selectedSeasonName ?? 'All Seasons' }} |
                                {{ \App\Models\district::find($selectedDistrict)?->name ?? 'All IP' }}
                            </span>
                        @else
                            <span class="font-semibold text-slate-900 dark:text-white">No collectors available</span>
                        @endif
                    </div>
                </div>
            </h2>
        ...
        {{-- <x-dd.card title="All Island Pest Data Comparisons" class="bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
